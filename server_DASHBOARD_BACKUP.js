                ...
                if (wordpressImageUrl) {
                  ...
                  pinterestGalleryHtml += `
                    <div class="pinterest-item" style="text-align: center;">
                      <img src="${wordpressImageUrl}" alt="${safeImageTitle}" style="width: 100%; max-width: 250px; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" />
  ...
  try {
    ...
    for (const recipeId of recipeIds) {
      try {
        ...
        // Add Pinterest images to content if enabled (same as regular publish)
        ...
        if (wpSettings.include_pinterest_images) {
          try {
            ...
            if (pinterestImages && pinterestImages.length > 0) {
              ...
              // Get language-specific Pinterest messages (same as regular publish)
              const getPinterestMessages = (language) => {
                const messages = {
                  'English': {
                    title: '📌 Pinterest Images',
              ...
              // Get language from recipe settings
              ...
              for (let i = 0; i < pinterestImages.length; i++) {
                try {
                  const pinterestImage = pinterestImages[i];
                  console.log(`📌 [BULK] Processing Pinterest image ${i + 1}/${pinterestImages.length} for recipe ${recipeId}`);
                  ...
                  if (pinterestImage.image_path && fs.existsSync(pinterestImage.image_path)) {
                    try {
                      // Upload to WordPress media library
                      const uploadResult = await wpClient.uploadMedia(pinterestImage.image_path, pinterestImage.filename);
                      if (uploadResult && uploadResult.source_url) {
                        wordpressImageUrl = uploadResult.source_url;
                        console.log(`📌 [BULK] Pinterest image uploaded to WordPress: ${wordpressImageUrl}`);
                      }
                    } catch (uploadError) {
                      console.log(`📌 [BULK] Error uploading Pinterest image to WordPress:`, uploadError.message);
                    }
                  }
                  ...
                  if (wordpressImageUrl) {
                    // Show Pinterest image (WordPress URL)
                    pinterestGalleryHtml += `
                      <div class="pinterest-item" style="text-align: center;">
                        <img src="${wordpressImageUrl}" alt="${safeImageTitle}" style="width: 100%; max-width: 250px; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);" />
                        <h4 style="color: #E60023; margin: 10px 0 5px 0; font-size: 14px;">${safeImageTitle}</h4>
              ...
              // Insert Pinterest gallery at a good position in the content (same logic as regular publish)
              if (content && typeof content === 'string') {
                let insertionPoint = -1;
  ...
  try {
    ...
    if (startDate || endDate) {
      ...
      console.log('Date filter applied:', {
        ...
        startDateLocal: dateFilter.startDate?.toLocaleString(),
        endDateLocal: dateFilter.endDate?.toLocaleString()
      });
    }
