...
class PinterestImageGenerator {
  ...
  async createCanvasTextOverlay(text, width, height, dominantColor = { r: 255, g: 120, b: 0 }, variation = 1) {
    ...
    fontSize = 42; // Fixed 42px font size
  ...
  async generateFromRecipe(recipeData, db, variation = 1) {
    ...
    try {
      ...
      // If only 1 image and it's a grid, try to crop individual images from it
      if (recipeImages.length === 1) {
        const imagePath = recipeImages[0].image_path;
        console.log(`📸 Only 1 image found: ${imagePath}`);
        
        // Check if it's a grid image (usually named with 'grid_' prefix)
